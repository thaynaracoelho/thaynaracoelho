# 👩🏽‍💻 I'm Thaynara Coelho 

**`Economist 🟣 Data Analyst 🟣 Business & Performance Analytics`**

Economist by training, data analyst, and a new mom — increasingly fascinated by the world of data and AI.

I enjoy exploring datasets, structuring metrics, and uncovering what the numbers really say about business performance.

My work and projects focus on using SQL, Python, and BI tools to translate data into insights that drive performance, efficiency, and decision-making.

I’m particularly interested in product analytics, performance, and data-driven business strategy in tech-driven environments.

This GitHub is where I share projects connecting **data, business, and strategy**.

<!-- Social icons section -->
<p align="center">
  <a href="https://www.linkedin.com/in/thaynara-coelho/"><img width="32px" alt="LinkedIn" title="LinkedIn" src="https://img.icons8.com/?size=100&id=8808&format=png&color=A04EE0"/></a>
  &#8287;&#8287;&#8287;&#8287;&#8287;
  <a href="https://www.instagram.com/eucoelhothay/"><img width="32px" alt="Instagram" title="Instagram" src="https://img.icons8.com/?size=100&id=59813&format=png&color=A04EE0"/></a>
  &#8287;&#8287;&#8287;&#8287;&#8287;
    <a href="mailto:thaynara.scoelho@outlook.com"><img width="32px" alt="e-mail" title="Send me an e-mail" src="https://img.icons8.com/?size=100&id=Xqc1euf6lDxs&format=png&color=A04EE0"></a>
  &#8287;&#8287;&#8287;&#8287;&#8287;
  <a href="futurechannel"><img width="32px" alt="Youtube" title="Youtube" src="https://img.icons8.com/?size=100&id=37326&format=png&color=A04EE0"/></a>
  &#8287;&#8287;&#8287;&#8287;&#8287;


<br/>

## 🔍 I am currently working on
* Building an end-to-end analytics case for a Foodtech platform, using AI-generated data to simulate real-world scenarios and drive insights on operational performance, revenue, and user behavior.
<br/>

## 🧩 Currently learning
* Currently deepening my skills in SQL, Python, and data modeling, focusing on building end-to-end analytics solutions that connect data, business strategy, and AI to drive performance and decision-making.
<br/>


## 🤖 Skillset


#### 🛠 Technical Skills
<p>
  <img alt="PostgreSQL" src="https://custom-icon-badges.demolab.com/badge/PostgreSQL | Intermadiate-DEDEDE.svg?logo=postgresqltha&logoColor=white"></a>
  <img alt="Python" src="https://custom-icon-badges.demolab.com/badge/Python | Numpy, Pandas and Matplotlib-03599C.svg?logo=icons8-python&logoColor=white"></a>  
  <img alt="PowerBI" src="https://custom-icon-badges.demolab.com/badge/Power BI | Advanced-FCC419.svg?logo=testethay&logoColor=white"></a>
  <img alt="Excel" src="https://custom-icon-badges.demolab.com/badge/Excel | Advanced-169154.svg?logo=icons8excel&logoColor=white"></a>
</p>

---

#### 📊 Data & Analytics
<p>
  <img src="https://img.shields.io/badge/Exploratory Data Analysis (EDA)-A04EE0?style=flat" alt="Statistical Analysis Skill Badge">
  <img src="https://img.shields.io/badge/KPI%20Development & Business Metrics-A04EE0?style=flat" alt="KPI Development Skill Badge">
  <img src="https://img.shields.io/badge/Data Storytelling-A04EE0?style=flat" alt="Data Storytelling Skill Badge"><br/>
  <img src="https://img.shields.io/badge/Statistical%20Analysis & Hypothesis Testing-0A8645?style=flat" alt="Statistical Analysis Skill Badge">
  <img src="https://img.shields.io/badge/Regression & Econometric Modeling-0A8645?style=flat" alt="Regression Analysis Skill Badge">
</p>
